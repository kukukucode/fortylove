...
import { Archive, BadgeCheck, Bot, CalendarDays, CircleHelp, LogOut, Menu, Settings, ShieldCheck, TicketCheck, UsersRound } from "lucide-react";
...
export function AdminNav({ role }: { role: "admin" | "super_admin" }) {
  ...
  return <aside className="admin-nav">
    ...
    <details className="admin-mobile-menu"><CloseMenuOnNavigation /><summary><Brand /><span><Menu />メニュー</span></summary><nav>{links}</nav><form action={logout} className="mobile-logout-form"><ConfirmSubmitButton message="ログアウトしますか？"><LogOut />ログアウト</ConfirmSubmitButton></form></details>
  </aside>;
}
